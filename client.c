...
void *client_thread_func(void *arg) {
  ...
  printf("[Client] start client thread %ld\n", thread_id);
  while (ops_count < TOT_NUM_OPS) {
    ...
    check(ret == 0, "[Client] failed to post write, curr opcount: [%lu]\n",
          ops_count);
...
int run_client() {
  ...
  for (i = 0; i < num_threads; i++) {
    ret = pthread_create(&client_threads[i], &attr, client_thread_func,
                         (void *)i);
